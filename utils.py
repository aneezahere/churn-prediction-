    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                'bgcolor': "rgba(0,0,0,0)",  # Match the dark theme
                'borderwidth': 2,
                'bordercolor': 'white',
                'steps': [
                    {'range': [0, 30], 'color': 'rgba(0, 255, 0, 0.5)'},
                    {'range': [30, 60], 'color': 'rgba(255, 255, 0, 0.5)'},
...
def create_model_probability_chart(probabilities):
    ...
    fig = go.Figure(data=[
        go.Bar(x=probs, y=models, orientation='h',
               text=[f'{p:.2%}' for p in probs],
               textposition='auto',
               marker=dict(color='rgb(58, 71, 80)'))  # Darker color bars for models
    ])
